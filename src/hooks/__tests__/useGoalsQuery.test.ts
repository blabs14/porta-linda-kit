...
import { useGoals, useCreateGoal, useUpdateGoal, useDeleteGoal, useAllocateFunds } from '../useGoalsQuery';
...
describe('useGoalsQuery', () => {
  ...
  describe('useAllocateFunds', () => {
    it('should allocate funds successfully', async () => {
      ...
      mockGoalsService.allocateFunds.mockResolvedValue({
        data: updatedGoal,
        error: null,
      });

      const { result } = renderHook(() => useAllocateFunds(), {
        wrapper: createWrapper(),
      });

      result.current.mutate(allocationData);
      ...
      expect(result.current.data).toEqual(updatedGoal);
      expect(mockGoalsService.allocateFunds).toHaveBeenCalledWith(
        allocationData.goalId,
        allocationData.amount
      );
    });

    it('should handle allocate funds error', async () => {
      const allocationData = {
        goalId: 'goal-1',
        amount: 500,
      };
      ...
      mockGoalsService.allocateFunds.mockResolvedValue({
        data: null,
        error: { message: 'Failed to allocate funds' },
      });
